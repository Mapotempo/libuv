...
describe Libuv::TTY do
  let(:handle_name) { :tty }
  let(:loop) { double() }
  let(:pointer) { double() }
  subject { Libuv::TTY.new(loop, pointer) }

  it_behaves_like 'a handle'
  it_behaves_like 'a stream'

  describe "#enable_raw_mode" do
    it "calls Libuv::Ext.tty_set_mode" do
      Libuv::Ext.should_receive(:tty_set_mode).with(pointer, 1)
  ...
  describe "#disable_raw_mode" do
    it "calls Libuv::Ext.tty_set_mode" do
      Libuv::Ext.should_receive(:tty_set_mode).with(pointer, 0)
  ...
  describe "#reset_mode" do
    it "calls Libuv::Ext.tty_reset_mode" do
      Libuv::Ext.should_receive(:tty_reset_mode)
  ...
  describe "#winsize" do
    ...
    it "calls Libuv::Ext.tty_get_winsize" do
      ...
      Libuv::Ext.should_receive(:tty_get_winsize).with(pointer, width_pointer, height_pointer)
